...
class BaseSlickDAO(db: Database)(implicit ec: ExecutionContext) extends Tables with ImplicitLogContext {
  ...
  )(implicit rowToObject: R => T, rc: RequestContext): Future[Result[T]] = {
  ...
  def lookupGenericOption[R, T](
    queryHeadOption: DBIOAction[Option[R], NoStream, Nothing]
  )(implicit rowToObject: R => T, rc: RequestContext): Future[Option[T]] = {
  ...
  )(implicit rc: RequestContext): Future[Result[Int]] = {
  ...
  def lookupSequenceGeneric[R, T](
    querySeq: DBIOAction[Seq[R], NoStream, Nothing]
  )(implicit rowToObject: R => T, rc: RequestContext): Future[Result[Seq[T]]] = {
  ...
  def lookupSequenceGenericRawSequence[R, T](
    querySeq: DBIOAction[Seq[R], NoStream, Nothing]
  )(implicit rowToObject: R => T, rc: RequestContext): Future[Seq[T]] = {
  ...
  )(implicit rowToObject: R => T, rc: RequestContext): Future[Result[Seq[Z]]] = {
    val queryResult: Future[Seq[R]] = db.run(querySeq).trace("lookupSequenceGeneric")
    queryResult.map { res: Seq[R] =>
  ...
  )(implicit rowToObject: R => T, rc: RequestContext): Future[Result[Z]] = {
    val queryResult: Future[Seq[R]] = db.run(querySeq).trace("lookupSequenceGeneric")
    queryResult.map { res: Seq[R] =>
  ...
  )(implicit rowToObject: R => T, objectToRow: T => R, rc: RequestContext): Future[Result[T]] = {
  ...
  )(implicit rc: RequestContext): Future[Result[Boolean]] = {
    val result = for {
      ...
      dbDeleteResult <- db.run(delete)
                          .map { x =>
                            if (x != 0)
                              ...
                              rc.log.error("Database Error deleting entity")
                              Left(
                                DatabaseResult(
