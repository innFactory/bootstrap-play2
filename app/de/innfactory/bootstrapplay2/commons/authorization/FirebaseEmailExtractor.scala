package de.innfactory.bootstrapplay2.commons.authorization
...
class FirebaseEmailExtractor[A] @Inject(val parser: BodyParsers.Default, environment: Environment)(implicit
  ...
  /**
   * Get Email from request Takes the request header "Authorization" and extracts the JWT token, then decodes the token
